...
#         c.execute("INSERT INTO attendance (student_id, name, timestamp, date) VALUES (?, ?, ?, ?)", (int(pred_label), name, ts, today))
#         conn.commit()
#         conn.close()
...
#     conn = sqlite3.connect(DB_PATH)
#     c = conn.cursor()
#     q = "SELECT id, student_id, name, timestamp, date FROM attendance"
#     params = ()
#     if period == "daily":
#         today = datetime.date.today().isoformat()
